...
class LeaveMasterService:
    ...
    def create_leave_master(self, actor: User, payload: LeaveMasterCreateRequest) -> list[LeaveMasterResponse]:
        _ = actor
        ...
        responses: list[LeaveMasterResponse] = []
        for leave_master_id in created_ids:
            ...
            responses.append(self._to_response(loaded))
        return responses

    def list_leave_masters(self, actor: User) -> list[LeaveMasterResponse]:
        _ = actor
    ...
    def update_leave_master(
        ...
        loaded = self.leave_master_repository.get_by_id(updated.id)
        if loaded is None:
            raise NotFoundException("Leave master not found")
        return self._to_response(loaded)

    def delete_leave_master(self, actor: User, leave_master_id: int) -> None:
        _ = actor
        leave_master = self.leave_master_repository.get_by_id(leave_master_id)
        if leave_master is None:
            raise NotFoundException("Leave master not found")
